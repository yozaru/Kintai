...
card=`echo -n "$card" | sed s/idm//g`
card=`echo "$card" | sed s/"'"//g`
card=`echo -n "$card" | sed s/"\n"//g`
echo $card
...
#entry=`echo 'use Shukei_development;select check_at from timestores where idm = '"011303002D124514"' limit 1'|mysql -u root -pMalmen1203`
