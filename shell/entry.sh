...
idm=`echo 'use Shukei_development;select idm from kintais where id = '"$id"''|mysql -u root -pMalmen1203`
#idm値が一致する値の中で5時以降で一番早い数字を入れ、フラグをたてる
entry=`echo 'use Shukei_development;select check_at from timestores where idm = '"$idm"' limit 1'|mysql -u root -pMalmen1203`
...
#entry=`mysql -u root -pMalmen1203 Shukei_development -e "select check_at from timestores where idm = '"$idm"' limit 1"`
#entry=`mysql -u root -pMalmen1203 Shukei_development -e "select check_at from timestores where idm = '"$idm"' limit 1"`
echo $idm
echo $entry
id=$((id + 1))
